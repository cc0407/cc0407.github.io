...
export const ImageSwap: React.FC<props> = props => {
    return (
        <div
            className={
                `absolute h-full w-full z-n-1 overflow-hidden ` +
                props.className
            }
        >
            <img
                alt={props.desktopAlt || ''}
                src={props.desktopImg}
                className={
                    `smallImg min-w-full overflow-hidden object-cover absolute top-0 z-0 hidden 1024:block ` +
                    (props.fitToDivHeight ? `h-full` : ``)
                }
            />
            <img
                alt={props.mobileAlt || ''}
                src={props.mobileImg}
                className={
                    `smallImg min-w-full overflow-hidden object-cover absolute top-0 z-0 block 1024:hidden ` +
                    (props.fitToDivHeight ? `h-full` : ``)
                }
            />
        </div>
